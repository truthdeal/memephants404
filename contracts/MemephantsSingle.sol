...
contract Memephants is Ownable {
    // Events
    event ERC20Transfer(
    ...
    mapping (address => uint256[]) public previousOwned ; 
...
/// @notice Initialization function to set pairs / etc
    ...
    function _transfer(
        ...
        if (!whitelist[to]) {
            uint256 tokens_to_mint = (balanceOf[to] / unit) -
               ...
               /* if (previousOwned[to].length > 0) {
                    uint256[] memory indexes = searchForAvailableIds(to, tokens_to_mint);
    ...
    //function searchForAvailableIds(address _to , uint256 _tokenAmount) internal view returns (uint256[] memory _indexes) {}
